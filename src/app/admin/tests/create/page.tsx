...
export default function AdminCreateTestPage() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        const seriesRes = await testService.getTestSeries();
  ...
  // Filter questions by search text locally
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setMessage({ type: "error", text: res.message || "Failed to create mock test." });
      }
    } catch (err: any) {
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-slate-100 flex items-center gap-2">
            <Award className="h-6 w-6 text-amber-600" /> Create New Mock Test
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-xs">
            Configure mock test parameters, scoring rules, assign to test series, and add questions.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                {testSeriesList.length === 0 ? (
                  <p className="text-xs text-slate-400 pt-2">No Test Series available yet.</p>
                ) : (
        ...
        <div className="flex items-center justify-end gap-4 pt-2">
          ...
          >
            {submitting ? "Creating..." : "Save & Create Test"}
          </Button>
        </div>
      </form>
    </div>
